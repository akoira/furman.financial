...
import by.dak.furman.financial.service.export.excel.Export2Excel;
...
import static by.dak.furman.financial.swing.category.TreePathUtils.buildRequest;
import static by.dak.furman.financial.swing.category.TreePathUtils.convert;
...
public class CategoriesPanel extends ATreeTablePanel {
	...
	public void exportExcel() {
		...
		ExportRequest request = TreePathUtils.buildRequest(nodes);
	...
	public void exportData() {
		ExportRequest request =
				buildRequest(convert(getTreeTable().getTreeSelectionModel().getSelectionPaths()));
		if (request.isValid()) {
			File file = getAppConfig().getExportService().export(request);
			UIUtils.openFileDir(file);
		}
		refreshData();
	}
